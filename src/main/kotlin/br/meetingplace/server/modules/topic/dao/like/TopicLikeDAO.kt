...
object TopicLikeDAO {
    fun like(data: TopicSimpleOperator, topicMapper: TopicMapperInterface): Status {
        return if (transaction { User.select { User.id eq data.userID } }.firstOrNull() != null && transaction { Topic.select { Topic.id eq data.topicID } }.firstOrNull() != null)
    ...
    private fun checkLikeDislike(topicID: String, topicMapper: TopicMapperInterface, userID: String): Int {
        val opinions = transaction { TopicOpinions.select { (TopicOpinions.userID eq userID) and (TopicOpinions.topicID eq topicID)}.map { topicMapper.mapTopicOpinions(it)}.firstOrNull() }
