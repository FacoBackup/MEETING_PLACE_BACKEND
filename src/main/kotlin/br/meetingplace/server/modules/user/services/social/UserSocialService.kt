...
object UserSocialService {
    ...
    fun follow(data: RequestSocial, userSocialDAO:SI, communityMemberDAO: CMI, communityDAO: CI, userDAO: UI): Status {
        return try {
            when(data.community){
                true-> {
    ...
    fun unfollow(data: RequestSocial, userSocialDAO:SI, communityMemberDAO: CMI): Status {
        return try {
            when(data.community){
