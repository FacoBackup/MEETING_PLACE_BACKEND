...
object UpdateMember {
    ...
    fun promote(data: MemberOperator): Status {
        return try {
            when(data.community){
                true->{
                    if(transaction { CommunityMember.select { (CommunityMember.communityID eq data.subjectID) and (CommunityMember.userID eq data.userID) and (CommunityMember.admin eq true)} }.firstOrNull() != null)
                        transaction {
