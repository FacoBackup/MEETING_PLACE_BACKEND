...
object ConversationMemberService {
    suspend fun addMember(requester: Long, data: RequestConversationMember, conversationMemberDAO: CMI, conversationDAO: CI): HttpStatusCode {
        return try{
    ...
    suspend fun removeMember(requester: Long, data: RequestConversationMember, conversationMemberDAO: CMI, conversationDAO: CI): HttpStatusCode {
        return try {
    ...
    suspend fun promoteMember(requester: Long, data: RequestConversationMember, memberDAO: CMI, conversationDAO: CI): HttpStatusCode{
        return try {
    ...
    suspend fun lowerMember(requester: Long, data: RequestConversationMember, memberDAO: CMI, conversationDAO: CI): HttpStatusCode{
        return try {
            val member = memberDAO.read(requester, conversationID = data.conversationID)
            val conversation = conversationDAO.read(data.conversationID)
            if(conversation != null && conversation.isGroup && member != null && member.admin)
