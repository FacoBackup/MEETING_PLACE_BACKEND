...
class Moderator private constructor() {
    ...
    fun approveTopic(data: Approval, communityDB: CommunityDBInterface, userDB: UserDBInterface, topicDB: TopicDBInterface) {
        ...
        if (user != null && community != null && getMemberRole(community.getMembers(), data.login.email) == MemberType.MODERATOR && !data.identifier.owner.isNullOrBlank() && topic != null && !topic.getApproved()) {
    ...
    fun approveGroup(data: Approval, communityDB: CommunityDBInterface, userDB: UserDBInterface, groupDB: GroupDBInterface) {
        ...
        if (user != null && community != null && getMemberRole(community.getMembers(),data.login.email) == MemberType.MODERATOR && group != null && !group.getApproved()) {
