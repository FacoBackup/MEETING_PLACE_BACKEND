package br.meetingplace.server.services.topic.controller.like
...
class LikeTopic private constructor() {
    ...
    fun like(data: TopicOperationsData, rwUser: UserLSInterface, rwTopic: TopicLSInterface, rwCommunity: CommunityLSInterface) {
    ...
    private fun likeUserMainTopic(data: TopicOperationsData, rwUser: UserLSInterface, rwTopic: TopicLSInterface) {
        val user = rwUser.load(data.login.email)
        val topic = rwTopic.load(data.identifier.mainTopicID, null)
        val creator = rwUser.load(data.identifier.mainTopicOwner)
    ...
    private fun likeUserSubTopic(data: TopicOperationsData, rwUser: UserLSInterface, rwTopic: TopicLSInterface) {
        val user = rwUser.load(data.login.email)
    ...
    private fun likeCommunityMainTopic(data: TopicOperationsData, rwTopic: TopicLSInterface, rwUser: UserLSInterface, rwCommunity: CommunityLSInterface) {
        val user = rwUser.load(data.login.email)
        val topic = rwTopic.load(data.identifier.mainTopicID, null)
        val creator = rwUser.load(data.identifier.mainTopicOwner)
    ...
    private fun likeCommunitySubtopic(data: TopicOperationsData, rwUser: UserLSInterface, rwCommunity: CommunityLSInterface, rwTopic: TopicLSInterface) {
        val user = rwUser.load(data.login.email)
        val subTopic = data.identifier.subTopicID?.let { rwTopic.load(it, data.identifier.mainTopicID) }
        val creator = rwUser.load(data.identifier.mainTopicOwner)
        val community = data.communityID?.let { rwCommunity.load(it) }
