package br.meetingplace.server.services.chat.controller.share
...
class ShareMessage private constructor() {
    ...
    fun shareMessage(data: ChatComplexOperator, rwUser: UserLSInterface, rwGroup: GroupLSInterface, rwCommunity: CommunityLSInterface, rwChat: ChatLSInterface){
        ...
        if(user != null){
            when(data.source.userGroup || data.source.communityGroup){
                ...
                false->{ //USER <-> USER
                    val chat = rwChat.load(data.source.chatID)
                    if(chat != null){
                        val messageContent = chat.shareMessage(data)
                        SendMessage.getClass().sendMessage(MessageData("|Shared| $messageContent", MessageType.SHARED, data.receiver, data.login), rwUser, rwGroup, rwCommunity, rwChat)
                        rwChat.store(chat)
